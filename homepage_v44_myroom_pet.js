...
.v44-creature-eye{position:absolute;top:48px;width:13px;height:18px;border-radius:50%;background:#35200f;box-shadow:inset 0 4px 0 rgba(255,255,255,.75)}.v44-creature-eye.left{left:55px}.v44-creature-eye.right{right:55px}
...
.v44-creature-wing{display:none;position:absolute;top:54px;width:42px;height:55px;border:3px solid rgba(255,255,255,.5);background:color-mix(in srgb,var(--pet-light) 70%,transparent)}.v44-creature-wing.left{left:2px;border-radius:70% 25% 70% 30%;transform:rotate(-24deg)}.v44-creature-wing.right{right:2px;border-radius:25% 70% 30% 70%;transform:rotate(24deg)}
...
`;
    ...
    function addMenuItems() {
        if (!document.querySelector(`.menu-item[data-page="${ROOM_PAGE}"]`)) {
            const collectionButton = document.querySelector('.menu-category[data-category="COLLECTION"]');
            const group = collectionButton?.nextElementSibling;
            if (group?.classList.contains("menu-group")) {
                group.appendChild(createMenuItem(ROOM_PAGE, "🏠", "MY ROOM"));
            }
        }
        if (!document.querySelector(`.menu-item[data-page="${PET_PAGE}"]`)) {
            const adventureButton = document.querySelector('.menu-category[data-category="ADVENTURE"]');
            const group = adventureButton?.nextElementSibling;
            if (group?.classList.contains("menu-group")) {
                group.appendChild(createMenuItem(PET_PAGE, "✦", "RECORD PET"));
            }
        }
        refreshNewBadgesV44();
    }
...
<div class="v44-creature stage-${evolution.stage} affinity-${evolution.affinity} ${scaleClass}" aria-label="${escapeHtml(evolution.name)}">
    <span class="v44-creature-wing left"></span><span class="v44-creature-wing right"></span>
    <div class="v44-creature-body"></div>
    <span class="v44-creature-eye left"></span><span class="v44-creature-eye right"></span>
...
<section class="v44-pet-shell" data-v44-page="pet">
        ...
        <div class="v44-pet-info">
            ...
            <div class="v44-care-grid">
                ...
                <button class="v44-care-button" data-v44-soul-snack type="button">
                    ...
                    <b>SOULのおやつ</b><small>10 SOULを使い、元気・親密度・経験値をまとめて増やします。</small>
                </button>
            </div>
    ...
    function renderPetPage(message = "") {
        ...
        setText("v44CareRemaining", Math.max(0, 8 - petData.daily.careCount));
        renderAffinityGrid();
        updateFloatingPet();
        updateDashboardCompanion();
    }
    ...
    function giveSoulSnack() {
        ...
        if (soul < 10) {
            showPetMessage("SOULが10必要です。ガチャの重複カードなどでSOULを集められます。", "error");
            if (typeof playUiSound === "function") playUiSound("error");
            return;
        }
        try {
