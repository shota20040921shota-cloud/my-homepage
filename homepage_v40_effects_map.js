...
(() => {
    ...
    const mapEntries = {
        ...
        shibuya: {
            ...
            label: "STUDY AREA",
            period: "大学生活",
            text: "國學院大學で法律を学ぶ活動地点。企画、情報社会、国際法などの学びを積み重ねています。",
            relatedPage: "SKILL",
        ...
        <section class="life-map-card" aria-labelledby="lifeMapTitle">
            ...
            <div class="life-map-stage">
                ...
                <button class="life-map-point selected" style="--x:58%;--y:48%" data-map-id="nishikasai" type="button">
                    <span class="life-map-dot"></span><span>西葛西</span>
                </button>
            ...
            <article id="lifeMapDetail" class="life-map-detail" aria-live="polite">
                ...
                <div class="life-map-detail-main">
                    ...
                    <h4 id="lifeMapDetailTitle">東京都 西葛西</h4>
...
@media(prefers-reduced-motion:reduce){
    .v40-transition-overlay,.v40-transition-overlay *,.life-map-compass,.life-map-main-line,.life-map-branch-line,.life-map-walk-line,.life-map-point.selected .life-map-dot::after{animation-duration:.01ms!important;animation-iteration-count:1!important}
}
`;
    ...
    function runTransition(pageName) {
        ...
        const record = document.querySelector(".record");
        if (record) {
            overlay.style.top = `${record.scrollTop}px`;
            overlay.style.bottom = "auto";
            overlay.style.height = `${record.clientHeight}px`;
        }
    ...
    function installTransitionTriggers() {
        document.addEventListener("pointerdown", event => {
            const menuItem = event.target.closest?.(".menu-item[data-page]");
            if (menuItem) runTransition(menuItem.dataset.page);

            if (event.target.closest?.("#locationPage") && typeof currentPageName !== "undefined") {
                runTransition(currentPageName);
            }
        }, true);
        ...
        window.addEventListener("keydown", event => {
            ...
            if (selected) runTransition(selected.dataset.page);
        }, true);
    }
