...
<div class="item-card">
    ...
    function calculateProfileStats() {
        ...
        let level = now.getFullYear() - birthYear;
        ...
        const days = Math.max(0, Math.floor((todayUtc - birthUtc) / 86400000));

        return { level: Math.max(0, level), days };
    }

    function updateDashboardValues() {
        if (!document.querySelector(".v41-dashboard")) return;

        const calculated = calculateProfileStats();
        const legacyLevel = document.getElementById("level")?.textContent?.trim() || "";
        const legacyDays = document.getElementById("days")?.textContent?.trim() || "";
        const level = /^\d+$/.test(legacyLevel) ? legacyLevel : String(calculated.level);
        const days = /^\d[\d,]*$/.test(legacyDays)
            ? legacyDays
            : calculated.days.toLocaleString("ja-JP");
        let owned = 0;
        let total = 170;
        let soul = 0;
